...
-   Part 1
    ...
    -   In the code chunk below create a **histogram** with `culmen_depth_mm` on the x axis and `color` by `species` using the dataframe *penguins_data*. Give the plot the theme: `theme_bw()`. Assign a color palette with `scale_fill_viridis_d()`. Give the plot a title, subtitle and custom x and y axis titles.
...
ggplot(data = penguins_data, aes(x = culmen_depth_mm, y = body_mass_g, color = species)) +
  ...
  labs(title = "Palmer Penguins, Culmen Depth by Body Mass",
       subtitle = "3 penguin species observed on three islands in the Palmer Archipelago, Antarctica",
       x = "Culmen Depth (mm)",
       y = "Body Mass (g)")
...
```{r, message=FALSE}
culmen_by_mass <- ggplot(data = penguins_data, aes(x = culmen_depth_mm, y = body_mass_g, color = species)) +
  geom_point() +
  theme_bw() + 
  scale_color_viridis_d(option="mako") +
  labs(title = "Palmer Penguins, Culmen Depth by Body Mass",
